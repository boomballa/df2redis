...
func NewShakeConfigStage() Stage {
	return StageFunc{
		...
		run: func(ctx *Context) Result {
			...
			targetCluster := strings.Contains(strings.ToLower(ctx.Config.Target.Type), "cluster")
			targetAddr := ctx.Config.Target.Seed
			targetPass := ctx.Config.Target.Password

			logPath := filepath.Join(ctx.StateDir, "shake.log")
...
`, filepath.Join(dir, file), targetCluster, targetAddr, targetPass, ctx.Config.Target.TLS, ctx.StateDir, logPath)
